 ...
 * @author admin
 */
public final class JdkTransformer implements ClassFileTransformer {

  /** . */
  private static final Logger LOGGER = Logger.getLogger(JdkTransformer.class.getName());
  ...
  private static void generateClass(final byte[] bytes, final String simpleName) {
    ...
    final String classPath =
        JdkTransformer.class.getResource("/").getPath() + simpleName + ".class";
  ...
  private static String assembleCode(
    ...
    code.append(Constants.LEFTBRACKET);
    // 打印内容开始.(使用JDK Date Api会报错,无法增强.具体问题需要再分析,当前系统时间戳)
    code.append("System.currentTimeMillis");
    code.append(Constants.LEFTBRACKET);
    code.append(Constants.RIGHTBRACKET);
    // 当前线程名 前缀修饰.
    code.append(codeFragment(Constants.LEFTMIDDLEBRACKET));
    // 当前线程名内容.
    code.append(Constants.PLUS);
    code.append("java.lang.Thread.currentThread().getName()");
    // 当前线程名 后缀修饰.
    code.append(codeFragment(Constants.RIGHTMIDDLEBRACKET));
    // 当前增强的类的全限定名和方法名.
    code.append(codeFragment(fqc + Constants.COLON + methodName + Constants.LEFTBRACKET));
    // 当前增强的类的方法参数.
    if (0 < argCount) {
      code.append(Constants.PLUS);
    ...
    // 方法结束部分.
    code.append(codeFragment(Constants.RIGHTBRACKET));
    // 方法返回值
  ...
  public static void enhanceMethod(final String fqc, final CtBehavior method) {
    try {
      ...
      if (!Modifier.isNative(modifiers) && !Modifier.isAbstract(modifiers)) {
        ...
        // 打印插入的代码详细信息.
        final String info = System.getProperty("info", Constants.TRUE);
        // true打印.
        if (Constants.TRUE.equals(info)) {
          LOGGER.info(code);
        }
        // 插入到方法的最后二行.
  ...
  public byte[] transform(
    ...
    try {
      ...
      final String packageName = System.getProperty("packageName", Constants.DEFPACKAGENAME);
      // 对某个包下的类进行处理.如果为空,默认对所有类进行增强,包括JDK中的类.
      if (Constants.DEFPACKAGENAME.equals(packageName) || fqClass.startsWith(packageName)) {
        //
        ...
        if (!ctclass.isInterface() && !ctclass.isEnum() && !ctclass.isAnnotation()) {
          ...
          if (Constants.TRUE.equals(generateClass)) {
